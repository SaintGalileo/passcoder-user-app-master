...
export default function AuthorizeBankTransferPayment({ route }) {
	const { sendOption, recipientDetails, sendPaymentAmount, sendPaymentNote, passUser } = route.params;
	...
	const authorizeSendPayment = async (pin, recipient, amount, note) => {
		const userToken = await AsyncStorage.getItem('userToken');
		fetch(`${BaseUrl}/user/transaction/bank/transfer`, {
			method: "POST",
			headers: {
			...
			body: JSON.stringify(!note ? {
				...
				bank: recipient.bank.bank_name,
				bank_code: recipient.bank.cbn_code,
				bank_short_code: recipient.bank.bank_code,
				first_name: passUser.firstname,
				last_name: passUser.lastname,
			...
			if (response.success) {
				...
				setSendPaymentSuccessMessage(response.message);
			} else {
	...
	function nextStep() {
		if (walletPin.length < 4) {
			...
			authorizeSendPayment(walletPin, recipientDetails, sendPaymentAmount, sendPaymentNote);
		}
	};
										...
										sendPaymentErrorMessage ?
											...
											<>
												...
												<View style={{ marginLeft: 15, marginRight: 15, marginBottom: 20 }}>
													...
													<Text style={{ fontFamily: 'lexendMedium', fontSize: 15, marginBottom: 5, justifyContent: "center", alignItems: "center", alignSelf: "center", marginLeft: 5, marginRight: 5 }}>{sendPaymentSuccessMessage}</Text>
