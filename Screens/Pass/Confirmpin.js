import { ActivityIndicator, Image, Keyboard, TouchableWithoutFeedback, KeyboardAvoidingView, StyleSheet, Text, TextInput, TouchableOpacity, View, Platform } from 'react-native'
...
export default function Confirmpin({ route }) {
    ...
    async function Addpin() {
        if (otpCode !== Pin) {
            Toast.show({
                type: "error",
                text1: "Error",
                text2: "Pin not matching!"
            })
        } else {
            setLoading(true);
            Keyboard.dismiss()
            const userToken = await AsyncStorage.getItem('userToken');
            fetch(`${BaseUrl}/user/pin/add`, {
                method: "POST",
                headers: {
                ...
                <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
                    <View style={styles.wrapper}>
                        ...
                        <View style={{ margin: 15 }}>
                            <Text style={{ fontFamily: "lexendBold", fontSize: 25, color: AppColor.Blue }}>Confirm PIN</Text>
                            <Text style={{ fontFamily: "lexendLight", marginTop: 5 }}>To protect the security of your PassCoder ID. Please confirm the 4 digit PIN.</Text>
                        </View>
                        ...
                        <View style={{ position: "absolute", bottom: 0, alignSelf: "center", marginBottom: Platform.OS === "ios" ? 20 : 20 }}>
                            <TouchableOpacity disabled={loading} onPress={Addpin} style={{ height: 50, width: 300, borderRadius: 8, backgroundColor: AppColor.Blue, justifyContent: "center", alignItems: "center" }}>
                                {loading ? (
