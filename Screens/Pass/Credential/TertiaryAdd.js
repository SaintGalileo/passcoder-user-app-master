...
export default function TertiaryAdd() {
	...
	async function UploadTertiary() {
		...
		if (!name) {
			...
			fetch(`${BaseUrl}/proofs/credentials/tertiary/institution`, {
				...
				fetch(`${BaseUrl}/user/tertiary/institution/add`, {
					method: "POST",
					headers: {
						'passcoder-access-token': userToken,
						'Accept': 'application/json',
			...
			<TouchableWithoutFeedback onPress={() => Keyboard.dismiss()}>
				<View style={styles.wrapper}>
						...
						<View style={{ margin: 15 }}>
							...
							<View style={{ marginBottom: 20 }}>
								...
								<TouchableOpacity onPress={pickProof} style={{ flexDirection: "row", backgroundColor: "#eee", height: 50, alignItems: "center", justifyContent: "space-between", borderRadius: 8 }}>
									<Text style={{ fontFamily: "lexendMedium", color: 'grey', paddingLeft: 20 }}>{preview?.extension ? `${preview?.extension + " - " + preview?.size + "MB"}` : "Select Document"}</Text>
					...
					<View style={{ position: "absolute", bottom: 0, alignSelf: "center", marginBottom: 20 }}>
						<TouchableOpacity onPress={UploadTertiary} disabled={uploading} style={styles.uploadButton}>
							{uploading ? (
