...
export default function TertiaryView() {
	...
	async function GetTertiary() {
		...
		fetch(`${BaseUrl}/user/tertiary/institutions`, {
			...
			setTertiary(response.data ? response.data.sort((a, b) => new Date(a.updatedAt.date + " " + a.updatedAt.time).getTime() < new Date(b.updatedAt.date + " " + b.updatedAt.time).getTime() ? 1 : -1) : []);
		}).catch((err) => {
			setLoading(false)
			Toast.show({
				type: 'error',
	...
	async function showSingleTertiary(uniqueId) {
		setLoading(true)
	...
	const onRefresh = React.useCallback(() => {
		setRefreshing(true);
		GetTertiary().then(() => { setRefreshing(false) }).catch(() => setRefreshing(false));
	}, []);

	async function DeleteTertiaryInstitution(id, name) {
	...
	const RenderTertiary = ({ item }) => {
		return (
			<>
				<TouchableOpacity onPress={() => { showSingleTertiary(item.unique_id) }} onLongPress={() => DeleteTertiaryInstitution(item?.unique_id, item?.name)} style={{ margin: 15 }}>
					<View style={{ backgroundColor: "#fff", padding: 20, borderRadius: 8, flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
						...
						<View style={{ flex: 1, marginLeft: 10 }}>
							...
							<Text style={{ fontFamily: "lexendMedium", color: "grey" }}>{item?.type} | {item?.degree}</Text>
				...
				isSingleDet
					...
					<View style={styles.wrapper}>
						...
						<View style={{ margin: 15, marginTop: 50, flexDirection: "row", justifyContent: "space-between", alignItems: "center" }}>
							...
							<View>
								...
								<Text style={{ textAlign: "center", fontFamily: "lexendLight" }}>Your Institution Certificate</Text>
							</View>
							<View>
								...
								<View style={{ margin: 15 }}>
									<View style={{ backgroundColor: "#fff", padding: 20, borderRadius: 8, flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
										...
										<View style={{ flex: 1, marginLeft: 10 }}>
											...
											<Text style={{ fontFamily: "lexendMedium", color: "grey" }}>{singleDet?.type} | {singleDet?.degree}</Text>
...

						{
							!singleDet?.verified && !loading ? (
								<View style={{ position: "absolute", bottom: 0, marginBottom: 20, alignSelf: "center", margin: 15 }}>
									<TouchableOpacity onPress={() => { nav.navigate('TertiaryEdit', singleDet) }} style={{ height: 45, backgroundColor: AppColor.Blue, borderRadius: 8, justifyContent: "center", alignItems: "center", width: 300 }}>
					...
					<View style={styles.wrapper}>
						...
						<View style={{ position: "absolute", bottom: 20, right: 10, margin: 10 }}>
							<TouchableOpacity onPress={() => nav.navigate('TertiaryAdd')} style={{ justifyContent: "center", backgroundColor: "#292482", borderRadius: 50, alignItems: 'center', marginTop: 50 }}>
