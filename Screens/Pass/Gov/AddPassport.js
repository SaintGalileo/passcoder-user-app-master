	...
	async function UploadPassport() {
		...
		// } 
			...
			fetch(`${BaseUrl}/proofs/government/passport`, {
				...
				fetch(`${BaseUrl}/user/passport/add`, {
					method: "POST",
					headers: {
						'passcoder-access-token': userToken,
						'Accept': 'application/json',
	...
	return (
		<View style={styles.wrapper}>
			...
			<View style={{ position: "absolute", bottom: 0, alignSelf: "center", marginBottom: 20 }}>
				<TouchableOpacity onPress={UploadPassport} disabled={uploading} style={{ backgroundColor: AppColor.Blue, borderRadius: 8, height: 50, width: 300, justifyContent: "center", alignItems: "center" }}>
					{
						uploading ? (
