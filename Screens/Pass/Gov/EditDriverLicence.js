...
export default function EditDriverLicence({ route }) {

	const { licence_number, issued_date, expiry_date, proof_front, proof_back, verified } = route.params;
	...
	async function UpdateDriverLicence() {
		...
		if (!licenceNumber) {
			...
			Toast.show({
				type: "error",
				text1: "Error",
				text2: "Front & Back of Licence is required"
			})
		} else {
			setUploading(true);
			...
			fetch(`${BaseUrl}/proofs/government/driver/licence`, {
				...
				fetch(`${BaseUrl}/user/driver/licence/edit/all`, {
					method: "PUT",
					headers: {
						'passcoder-access-token': userToken,
						'Accept': 'application/json',
			...
			<ScrollView style={{ marginBottom: 100 }}>
				<View style={{ margin: 15, marginTop: 30 }}>
...

					{
						proof_front ? 
							<View style={{ margin: 15, alignItems: "center", marginBottom: 20 }}>
								<Text style={{ fontFamily: "lexendMedium", marginBottom: 10 }}>Current Front of Licence</Text>
...

					{
						proof_back ? 
							<View style={{ margin: 15, alignItems: "center", marginBottom: 20 }}>
								<Text style={{ fontFamily: "lexendMedium", marginBottom: 10 }}>Current Back of Licence</Text>
...

			{
				verified ?
					...
					<View style={{ position: "absolute", bottom: 0, alignSelf: "center", marginBottom: 20 }}>
						<TouchableOpacity onPress={UpdateDriverLicence} disabled={uploading} style={{ backgroundColor: AppColor.Blue, borderRadius: 8, height: 50, width: 300, justifyContent: "center", alignItems: "center" }}>
							{
								uploading ? (
