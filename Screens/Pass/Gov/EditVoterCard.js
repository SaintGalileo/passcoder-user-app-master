...
export default function EditVoterCard({ route }) {

	const { vin, card_number, issued_date, expiry_date, proof_front, proof_back, verified } = route.params;
	...
	async function ChangeVoterCard() {
		...
		// } 
		...
		} else if ((proofBack && (!proofFront || proofFront.length === 0)) || (proofFront && (!proofBack || proofBack.length === 0))) {
			Toast.show({
				type: "error",
				text1: "Error",
				text2: "Front & Back of Document is required"
			})
		} else {
			setUploading(true);
			...
			fetch(`${BaseUrl}/proofs/government/voter/card`, {
				...
				fetch(`${BaseUrl}/user/voter/card/edit/all`, {
					method: "PUT",
					headers: {
						'passcoder-access-token': userToken,
						'Accept': 'application/json',
					...
					body: JSON.stringify({
						...
						card_number: cardNumber ? return_trimmed_data(cardNumber) : undefined,
					...
					if (res.status === 204) {
											...
											fetch(`${BaseUrl}/user/voter/card/edit/proofs`, {
												...
												if (upload_proof_res.success) {
													...
													Toast.show({
														type: "success",
														text1: "Success",
														text2: "Details & documents updated successfully!"
													})
													nav.goBack();
												} else {
													setUploading(false);
						...
						Toast.show({
							type: "error",
							text1: "Error",
							text2: res.status !== 422 ? error_res.message : error_res.data[0].msg
						});
					}
				})
			})
			...
			<ScrollView style={{ marginBottom: 100 }}>
				<View style={{ margin: 15, marginTop: 30 }}>
...

					{
						proof_front ? 
							<View style={{ margin: 15, alignItems: "center", marginBottom: 20 }}>
								<Text style={{ fontFamily: "lexendMedium", marginBottom: 10 }}>Current Front of Document</Text>
								<Image style={{ height: 200, width: 300 }} resizeMode='contain' source={{ uri: proof_front }} />
							</View> : ""
					}

					<View style={{ marginBottom: 20 }}>
						<Text style={{ fontFamily: "lexendBold", marginBottom: 10 }}>Front of Document</Text>
						<TouchableOpacity disabled={verified} onPress={pickProofFront} style={{ flexDirection: "row", backgroundColor: "#eee", height: 50, alignItems: "center", justifyContent: "space-between", borderRadius: 8 }}>
...

					{
						proof_back ? 
							<View style={{ margin: 15, alignItems: "center", marginBottom: 20 }}>
								<Text style={{ fontFamily: "lexendMedium", marginBottom: 10 }}>Current Back of Document</Text>
								<Image style={{ height: 200, width: 300 }} resizeMode='contain' source={{ uri: proof_back }} />
							</View> : ""
					}

					<View style={{ marginBottom: 20 }}>
						<Text style={{ fontFamily: "lexendBold", marginBottom: 10 }}>Back of Document</Text>
						<TouchableOpacity disabled={verified} onPress={pickProofBack} style={{ flexDirection: "row", backgroundColor: "#eee", height: 50, alignItems: "center", justifyContent: "space-between", borderRadius: 8 }}>
...

			{
				verified ?
					...
					<View style={{ position: "absolute", bottom: 0, alignSelf: "center", marginBottom: 20 }}>
						<TouchableOpacity onPress={ChangeVoterCard} disabled={uploading} style={{ backgroundColor: AppColor.Blue, borderRadius: 8, height: 50, width: 300, justifyContent: "center", alignItems: "center" }}>
							{
								uploading ? (
