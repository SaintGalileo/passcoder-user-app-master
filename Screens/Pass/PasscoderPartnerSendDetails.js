...
export default function PasscoderPartnerSendDetails({ route }) {
	const { sendOption, sendPaymentPID, recipientPartnerDetails, passUser } = route.params;
	...
	function nextStep() {
		if (!sendPaymentAmount) {
			...
			nav.navigate("AuthorizePasscoderPartnerPayment", {
						...
						<ScrollView style={{ marginBottom: 90, }} showsVerticalScrollIndicator={false}>
							<View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-evenly", marginLeft: 15, marginRight: 15 }}>
								<View style={{ height: 70, width: 70, borderRadius: 100, backgroundColor: "#eee", justifyContent: "center", alignItems: "center", alignSelf: "center", marginTop: 10 }}>
									<MaterialCommunityIcons name={"briefcase-account"} size={30} color={AppColor.Blue} />
								</View>
								<View style={{ marginLeft: 15, marginRight: 15, marginTop: 10 }}>
									<Text style={{ fontFamily: 'lexendMedium', fontSize: 17, marginBottom: 10, color: "#000", flexWrap: 'wrap', maxWidth: (Dimensions.get('screen').width * 70) / 100 }}>{recipientPartnerDetails.name + " (" + recipientPartnerDetails.city + ")"}</Text>
									<Text style={{ fontFamily: 'lexendBold', fontSize: 14, marginBottom: 10, color: "#000" }}>{recipientPartnerDetails.pid}</Text>
								</View>
							</View>
