...
export default function PasscoderUserSend({ route }) {
	const { sendOption, passUser } = route.params;

	const nav = useNavigation();

	const [sendPaymentPID, setSendPaymentPID] = useState("");
	const [recipientUserLoading, setRecipientUserLoading] = useState(false);
	const [recipientUserDetails, setRecipientUserDetails] = useState({});
	...
	async function nextStep() {
		if (sendPaymentPID === passUser.pid) {
			...
			setRecipientUserLoading(true);
			const userToken = await AsyncStorage.getItem('userToken');
			fetch(`${BaseUrl}/public/find/user`, {
				method: "POST",
				headers: {
				...
				if (response.success) {
					setRecipientUserLoading(false);
					setRecipientUserDetails(response.data);
					nav.navigate("PasscoderUserSendDetails", {
						sendOption: sendOption,
						sendPaymentPID: sendPaymentPID,
						passUser: passUser,
						recipientUserDetails: response.data
					});
				} else {
					setRecipientUserLoading(false);
					Toast.show({
			...
			<TouchableWithoutFeedback onPress={Keyboard.dismiss}>
				<View style={styles.wrapper}>
					...
					<View style={{ position: "absolute", bottom: 0, alignSelf: "flex-end", marginBottom: Platform.OS === "ios" ? 70 : 20 }}>
						<TouchableOpacity style={{ ...styles.btnStyle, backgroundColor: recipientUserLoading ? "grey" : AppColor.Blue }} disabled={recipientUserLoading} onPress={nextStep}>
							{recipientUserLoading ? (
