...
export default function PasscoderUserSendDetails({ route }) {
	const { sendOption, sendPaymentPID, recipientUserDetails, passUser } = route.params;
	...
	function nextStep() {
		if (!sendPaymentAmount) {
		...
		} else if (sendPaymentAmount < general_min_amount) {
			Toast.show({
				type: "error",
				text1: "Error",
				text2: `Minimum amount is ${digit_filter(general_min_amount)} Naira`
			});
			...
			nav.navigate("AuthorizePasscoderUserPayment", {
					...
					<ScrollView style={{ marginBottom: 90, }} showsVerticalScrollIndicator={false}>
						<View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-evenly", marginLeft: 15, marginRight: 15 }}>
							<View style={{ height: 70, width: 70, borderRadius: 100, backgroundColor: "#eee", justifyContent: "center", alignItems: "center", alignSelf: "center", marginTop: 10 }}>
								<MaterialCommunityIcons name={"account"} size={30} color={AppColor.Blue} />
							</View>
							<View style={{ marginLeft: 15, marginRight: 15, marginTop: 10 }}>
								<Text style={{ fontFamily: 'lexendMedium', fontSize: 20, marginBottom: 10, color: "#000", flexWrap: 'wrap', maxWidth: (Dimensions.get('screen').width * 70) / 100 }}>{recipientUserDetails.firstname + (recipientUserDetails.middlename ? " " + recipientUserDetails.middlename + " " : " ") + recipientUserDetails.lastname}</Text>
								<Text style={{ fontFamily: 'lexendBold', fontSize: 17, marginBottom: 10, color: "#000" }}>{recipientUserDetails.pid}</Text>
							</View>
						</View>
