...
export default function ProofAddress() {
	...
	async function UploadAddress() {
		...
		if (!address) {
			...
			fetch(`${BaseUrl}/proofs/bio/address${!userPID || userPID === "******" ? "/via/uid" : ""}`, {
				...
				fetch(`${BaseUrl}/user/address/add`, {
					method: "POST",
					headers: {
						'passcoder-access-token': userToken,
						'Accept': 'application/json',
					...
					body: JSON.stringify(!additionalInfo ? {
						...
						issued_date: _issued_date,
						proof_type: proofType,
						additional_information: return_trimmed_data(additionalInfo)
					})
				}).then(async (res) => {
		...
		<>
			...
			<View style={styles.wrapper}>
					...
					<View style={{ margin: 15 }}>
						...
						<View style={{ marginBottom: 20 }}>
							...
							<TouchableOpacity onPress={() => { setModalVisible(true) }} style={{ flexDirection: "row", backgroundColor: "#eee", height: 50, alignItems: "center", justifyContent: "space-between", borderRadius: 8 }}>
								<Text style={{ fontFamily: "lexendMedium", color: 'grey', paddingLeft: 20 }}>{proofType	|| "Select Proof Type"}</Text>
								<FontAwesome5 name="sort-down" size={24} color="grey" style={{ marginRight: 10, paddingBottom: 10 }} />
							</TouchableOpacity>
						</View>
				...
				<View style={{ position: "absolute", bottom: 0, alignSelf: "center", marginBottom: 20 }}>
					<TouchableOpacity onPress={UploadAddress} disabled={uploading} style={styles.uploadButton}>
						{uploading ? (
